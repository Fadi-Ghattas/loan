        ...
        <div class="col-md-12">
            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    @if($loan->status=="disbursed" || $loan->status=="closed" || $loan->status=="withdrawn" || $loan->status=="written_off" || $loan->status=="rescheduled" )
                        <li class=""><a href="#repayments" data-toggle="tab" aria-expanded="true">{{trans_choice('general.repayment',2)}}</a></li>

                        <li><a href="#loan_schedule" data-toggle="tab" aria-expanded="false">{{trans_choice('general.loan',1)}} {{trans_choice('general.schedule',1)}}</a></li>


                        <li class=""><a href="#pending_dues" data-toggle="tab" aria-expanded="false">{{trans_choice('general.pending',1)}} {{trans_choice('general.due',2)}}</a></li>
                                ...
                                <div class="col-sm-9 pull-right">
                                    <div class="btn-group-horizontal">
                                        ...
                                        @if(Sentinel::hasAccess('loans.update'))
                                            ...
                                            <a type="button" class="btn bg-gray margin"
                                               href="{{url('loan/'.$loan->id.'/payment-on-account/create')}}">{{trans_choice('general.add',1)}}
                            ...
                            <div class="box box-success">
                                <div class="box-body table-responsive no-padding">
                                    <table class="table table-bordered table-condensed table-hover">
                                        ...
                                        ?>
                                        <tr class="@if(( ceil($schedule->principal+$schedule->interest+$schedule->fees+$schedule->penalty) - ceil(floatval(\App\Models\LoanRepayment::where('loan_id',$loan->id)->where('due_date',$schedule->due_date)->sum('amount'))) )<=0) success @endif">
                                            <td>
                                                {{$count}}
                                            </td>
                                            <td>
                                            ...
                                            <td style="text-align:right;">
                                                {{number_format(ceil($schedule->principal+$schedule->interest+$schedule->fees+$schedule->penalty) - ceil(floatval(\App\Models\LoanRepayment::where('loan_id',$loan->id)->where('due_date',$schedule->due_date)->sum('amount')))  ,2)}}
                                            </td>
                                            <td style="text-align:right;">
